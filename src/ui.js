...
export function installDelegation() {
  document.addEventListener("input", (e) => {
    ...
    if (el) runAttr(el.getAttribute("input") || "", e);
  });
...
export function openProviderModal(m) {
    ...
    <div class="pv-panel">
      ...
      <div class="pv-list">
        ...
        <button class="pv-item pv-manual">
          <span class="pv-name">${t("prov.manual")}</span>
          <span class="pv-arrow">→</span>
        </button>
      </div>
  ...
  mask.querySelectorAll(".pv-item[data-id]").forEach((b) => {
    b.addEventListener("click", () => { close(); m.onPick?.(b.dataset.id); });
  });
  mask.querySelector(".pv-manual").addEventListener("click", () => { close(); m.onManual?.(); });
