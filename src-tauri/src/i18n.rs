...
const ZH: &[(&str, &str)] = &[
    ...
    ("err.oauth.init_invalid", "官方 OAuth 初始化响应缺少必要字段，请稍后重试；也可在 ZCode 登录后使用「保存登录」"),
    ("err.oauth.init_http", "官方 OAuth 初始化失败（HTTP {status}）：{msg}"),
    ("err.oauth.init_server_unknown", "服务端未提供原因"),
...
const EN: &[(&str, &str)] = &[
    ...
    ("title.settings", "Z-Accounts Settings"),
    ...
    ("err.oauth.init_invalid", "The official OAuth init response is missing required fields. Retry later, or sign in through ZCode and use Save login"),
    ("err.oauth.init_http", "Official OAuth init failed (HTTP {status}): {msg}"),
    ("err.oauth.init_server_unknown", "no reason provided by the server"),
