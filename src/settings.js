...
function render() {
  ...
  const s = state;
  document.title = `Z-Accounts · ${t("s.title")}`;
  $app.innerHTML = `
    <header class="topbar">
      <div class="brand"><img class="brand-mark" src="/brand-icon.png" alt=""><div class="brand-copy"><span class="wordmark">Z-Accounts</span><span class="brand-caption">${t("s.title")}${appVer ? ` · v${esc(appVer)}` : ""}</span></div></div>
    </header>
    <section class="settings open">
      ${langSeg(s.language || "zh")}
      <div class="tog-row"><div class="tog-info"><div class="tog-label">${t("m.appearance")}</div></div>
        <div class="lang-seg compact" role="radiogroup" aria-label="${t("m.appearance")}">
          <button class="lang-opt${s.theme !== "light" ? " on" : ""}" role="radio" aria-checked="${s.theme !== "light"}" click="actions.setTheme('dark')">${ic("moon", 15)} ${t("m.darkMode")}</button>
          <button class="lang-opt${s.theme === "light" ? " on" : ""}" role="radio" aria-checked="${s.theme === "light"}" click="actions.setTheme('light')">${ic("sun", 15)} ${t("m.lightMode")}</button>
        </div>
      </div>
