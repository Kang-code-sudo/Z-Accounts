...
pub(crate) fn device_mid() -> Option<String> {
    ...
    CACHE
        .get_or_init(|| {
            let p = crate::store::Paths::detect().live_telemetry();
...
pub(crate) enum ConversationAvailability {
    Available,
    Exhausted,
    Unknown,
}

fn numeric_remaining(total: Option<f64>, used: Option<f64>, remaining: Option<f64>) -> Option<f64> {
    remaining.filter(|v| v.is_finite()).or_else(|| {
        match (total, used) {
            (Some(t), Some(u)) if t.is_finite() && t > 0.0 && u.is_finite() => Some((t - u).max(0.0)),
            _ => None,
        }
    })
}

/// Only a measured balance is actionable. A grant without current usage is not
/// proof that the account can chat, and a failed/incomplete response is not
/// proof that it has run out.
pub(crate) fn conversation_availability(q: &QuotaOverview) -> ConversationAvailability {
    let bigmodel = q.source == "bigmodel.cn/api/monitor";
    let mut positive = false;
    let mut depleted = false;
    let mut unknown = false;
    let mut required_depleted = false;
    let mut required_unknown = false;

    if !q.items.is_empty() {
        for item in &q.items {
            if item.kind.as_deref() == Some("grant") { continue; }
            let required = bigmodel && matches!(item.kind.as_deref(), Some("prompt_count" | "duration"));
            match numeric_remaining(item.total, item.used, item.remaining) {
                Some(v) if v > 0.0 => positive = true,
                Some(_) => {
                    depleted = true;
                    required_depleted |= required;
                }
                None => {
                    unknown = true;
                    required_unknown |= required;
                }
            }
        }
    } else if !q.plans.is_empty() {
        for plan in &q.plans {
            match numeric_remaining(plan.total, plan.used, plan.remaining) {
                Some(v) if v > 0.0 => positive = true,
                Some(_) => depleted = true,
                None => unknown = true,
            }
        }
    } else {
        match numeric_remaining(q.total, q.used, q.remaining) {
            Some(v) if v > 0.0 => positive = true,
            Some(_) => depleted = true,
            None => unknown = true,
        }
...
/// ZCode stores account-scoped coding-plan keys under credential names such as
/// `account-provider:coding-plan:...:api-key`. They are encrypted separately
/// from provider.options.apiKey, so imported accounts need this fallback.
fn credential_api_keys(creds: &Value, secret: &str) -> Vec<(String, String)> {
    let mut keys = vec![];
    let Some(map) = creds.as_object() else { return keys };
    for (name, value) in map {
        let lower = name.to_ascii_lowercase();
        if !(lower.ends_with(":api-key") || lower.ends_with(":apikey") || lower.ends_with(":api_key")) {
            continue;
        }
        let Some(raw) = value.as_str() else { continue };
        let Some(token) = safe_decrypt(Some(raw), secret) else { continue };
        if looks_like_token(&token) && !keys.iter().any(|(_, existing)| existing == &token) {
            keys.push((name.clone(), token));
        }
    }
    keys
}
...
pub fn candidate_tokens(creds: &Value, config: Option<&Value>, secret: &str) -> Vec<String> {
    ...
    for (_, k) in credential_api_keys(creds, secret) {
...
fn query_with_token_via(token: &str, fetch: &FetchFn) -> Result<QuotaOverview, String> {
    ...
    let mut partial_monitor: Option<QuotaOverview> = None;
    ...
    match fetch(QUOTA_LIMIT_URL, token) {
        Ok(limit_resp) => {
            if business_ok(&limit_resp) {
                ...
                if quota_has_measured_balance(&ov) {
                    return Ok(ov);
                }
                partial_monitor = Some(ov);
            }
    ...
    match fetch(&url, token) {
        Ok(balance) if business_ok(&balance) => {
            ...
            if quota_has_content(&ov) {
                return Ok(ov);
            }
            return Ok(partial_monitor.unwrap_or(ov));
        }
        Ok(_) => {}
        Err(e) => {
            if best_err.is_none() {
    ...
    partial_monitor.ok_or_else(|| best_err.unwrap_or_else(|| crate::i18n::tr("err.quota.fail")))
}
...
pub fn query_quota(tokens: &[String]) -> Result<QuotaOverview, String> {
    ...
    let mut partial_overview: Option<QuotaOverview> = None;
    let mut empty_overview: Option<QuotaOverview> = None;
    let mut auth_fail = 0usize;
    for t in tokens {
        match query_with_token(t) {
            Ok(ov) => {
                // A valid token may still return an empty response while a
                // second account-scoped key contains the actual plan.
                if quota_has_measured_balance(&ov) {
                    return Ok(ov);
                }
                if quota_has_content(&ov) {
                    partial_overview.get_or_insert(ov);
                } else {
                    empty_overview.get_or_insert(ov);
                }
            }
            Err(e) => {
                if e.contains("401") {
    ...
    if let Some(ov) = partial_overview.or(empty_overview) {
        return Ok(ov);
    }
...
fn pick_channels(creds: &Value, config: Option<&Value>, secret: &str) -> Vec<Channel> {
    ...
    let providers = config.and_then(|c| c.get("provider")).and_then(|p| p.as_object());
    let mut ordered: Vec<(&String, &Value)> = providers.map(|p| p.iter().collect()).unwrap_or_default();
    ordered.sort_by_key(|(_, p)| if p.get("enabled").and_then(|e| e.as_bool()).unwrap_or(false) { 0 } else { 1 });
    ...
    // Imported accounts keep the coding-plan key in an encrypted,
    // account-scoped credential instead of provider.options.apiKey.
    for (name, token) in credential_api_keys(creds, secret) {
        if name.to_ascii_lowercase().contains("coding-plan") && !chans.contains(&Channel::Monitor(token.clone())) {
            chans.push(Channel::Monitor(token));
        }
    }
    // A partial snapshot may have credentials without a config snapshot.
    let jwt = safe_decrypt(creds.get("zcodejwttoken").and_then(|v| v.as_str()), secret);
    let active = safe_decrypt(creds.get("oauth:active_provider").and_then(|v| v.as_str()), secret);
    if active.as_deref() != Some("bigmodel") {
        if let Some(token) = jwt {
            if !chans.contains(&Channel::ZaiBilling(token.clone())) {
                chans.push(Channel::ZaiBilling(token));
            }
        }
    }
    chans
...
fn quota_has_content(ov: &QuotaOverview) -> bool {
    !ov.is_empty
        || ov.plan_tier.is_some()
        || !ov.plans.is_empty()
        || !ov.items.is_empty()
        || ov.total.is_some()
        || ov.remaining.is_some()
}

fn quota_has_measured_balance(ov: &QuotaOverview) -> bool {
    ov.items.iter().any(|item| item.kind.as_deref() != Some("grant")
        && numeric_remaining(item.total, item.used, item.remaining).is_some())
        || ov.plans.iter().any(|plan| numeric_remaining(plan.total, plan.used, plan.remaining).is_some())
        || numeric_remaining(ov.total, ov.used, ov.remaining).is_some()
}
...
pub fn quota_for_live(home: &Path, creds: &Value, config: Option<&Value>) -> Result<QuotaOverview, String> {
    ...
    let mut partial_channel: Option<QuotaOverview> = None;
    if !channels.is_empty() {
        match query_channels(&channels) {
            Ok(ov) if quota_has_measured_balance(&ov) => return Ok(ov),
            Ok(ov) => partial_channel = Some(ov),
            Err(_) => {}
        }
    }
    let tokens = candidate_tokens(creds, config, &secret);
    match query_quota(&tokens) {
        Ok(ov) if quota_has_measured_balance(&ov) => Ok(ov),
        Ok(ov) if quota_has_content(&ov) => Ok(partial_channel.filter(quota_has_content).unwrap_or(ov)),
        Ok(ov) => Ok(partial_channel.unwrap_or(ov)),
        Err(e) => partial_channel.ok_or(e),
    }
}
...
fn normalize_balance(balance_data: &Value) -> QuotaOverview {
    ...
    // Some newly activated plans expose their allowance in `entitlements` before
    // the billing API creates a `balances` record. Show the granted amount, but
    // do not invent a remaining balance or usage percentage.
    for (slot, plan) in slots.iter_mut().zip(
        balance.get("plans").and_then(Value::as_array).into_iter().flatten().filter(|pl| {
            pl.get("status").and_then(Value::as_str).map(|s| s.eq_ignore_ascii_case("active")).unwrap_or(false)
        }),
    ) {
        if !slot.items.is_empty() {
            continue;
        }
        if let Some(entitlements) = plan.get("entitlements").and_then(Value::as_array) {
            for entitlement in entitlements {
                let Some(total) = entitlement.get("grant_units").and_then(to_number) else { continue; };
                slot.items.push(QuotaItem {
                    name: entitlement.get("show_name").and_then(Value::as_str).unwrap_or("Quota").to_string(),
                    total: Some(total),
                    unit: entitlement.get("unit_type").and_then(Value::as_str).unwrap_or("quota").to_string(),
                    kind: Some("grant".into()),
                    ..Default::default()
                });
            }
        }
...
mod tests {
    ...
    #[test]
    fn empty_monitor_result_still_checks_billing() {
        let fetch = |url: &str, _token: &str| -> Result<Value, String> {
            if url == QUOTA_LIMIT_URL {
                Ok(json!({"code": 200, "data": {"limits": []}}))
            } else if url == SUBSCRIPTION_URL {
                Ok(json!({"code": 200, "data": []}))
            } else {
                Ok(json!({"code": 200, "data": {"plans": [{"status": "active", "plan_id": "start", "name": "Start Plan"}], "balances": []}}))
            }
        };
        let result = query_with_token_via("test-token", &fetch).unwrap();
        assert_eq!(result.source, "zcode.z.ai/billing");
        assert_eq!(result.plan_tier.as_deref(), Some("Start Plan"));
        assert!(!result.is_empty);
    }

    #[test]
    fn plan_only_monitor_does_not_hide_measured_billing_balance() {
        let fetch = |url: &str, _token: &str| -> Result<Value, String> {
            if url == QUOTA_LIMIT_URL {
                Ok(json!({"code": 200, "data": {"limits": [], "level": "START"}}))
            } else if url == SUBSCRIPTION_URL {
                Ok(json!({"code": 200, "data": []}))
            } else {
                Ok(json!({"code": 200, "data": {
                    "plans": [{"status": "active", "plan_id": "start", "name": "Start Plan"}],
                    "balances": [{"plan_id": "start", "show_name": "GLM-5.3", "unit_type": "token",
                        "total_units": 3000000, "used_units": 500000, "remaining_units": 2500000}]
                }}))
            }
        };
        let result = query_with_token_via("test-token", &fetch).unwrap();
        assert_eq!(result.source, "zcode.z.ai/billing");
        assert_eq!(result.items[0].remaining, Some(2500000.0));
    }

    #[test]
    fn active_plan_without_balance_is_known_but_remaining_is_unknown() {
        let response = json!({"code": 200, "data": {
            "plans": [{"status": "active", "plan_id": "start", "name": "Start Plan"}],
            "balances": []
        }});
        let q = normalize_balance(&response);
        assert!(!q.is_empty);
        assert_eq!(q.remaining, None);
        assert_eq!(conversation_availability(&q), ConversationAvailability::Unknown);
    }

    #[test]
    fn grant_only_plan_shows_allowance_without_inventing_remaining() {
        let response = json!({"code": 200, "data": {
            "plans": [{"status": "active", "plan_id": "start", "name": "Start Plan",
                "entitlements": [{"show_name": "GLM-5.3-Flash", "grant_units": 100000000,
                    "unit_type": "token"}]}],
            "balances": []
        }});
        let q = normalize_balance(&response);
        assert_eq!(q.items.len(), 1);
        assert_eq!(q.items[0].kind.as_deref(), Some("grant"));
        assert_eq!(q.items[0].total, Some(100000000.0));
        assert_eq!(q.remaining, None);
        assert_eq!(q.percent_used, None);
        assert!(!q.is_empty);
        assert!(!quota_has_measured_balance(&q));
        assert_eq!(conversation_availability(&q), ConversationAvailability::Unknown);
    }

    #[test]
    fn auto_switch_waits_for_measured_depletion_and_accepts_any_live_billing_balance() {
        let mut q = QuotaOverview {
            source: "zcode.z.ai/billing".into(),
            items: vec![
                QuotaItem { remaining: Some(0.0), ..Default::default() },
                QuotaItem { remaining: Some(3.0), ..Default::default() },
            ],
            ..Default::default()
        };
        assert_eq!(conversation_availability(&q), ConversationAvailability::Available);
        q.items[1].remaining = Some(0.0);
        assert_eq!(conversation_availability(&q), ConversationAvailability::Exhausted);
        q.items[1].remaining = None;
        assert_eq!(conversation_availability(&q), ConversationAvailability::Unknown);
        q.items[1] = QuotaItem { kind: Some("grant".into()), total: Some(100.0), ..Default::default() };
        assert_eq!(conversation_availability(&q), ConversationAvailability::Exhausted);
    }

    #[test]
    fn depleted_bigmodel_window_blocks_conversation_even_with_another_positive_window() {
        let q = QuotaOverview {
            source: "bigmodel.cn/api/monitor".into(),
            items: vec![
                QuotaItem { kind: Some("prompt_count".into()), remaining: Some(0.0), ..Default::default() },
                QuotaItem { kind: Some("duration".into()), remaining: Some(30.0), ..Default::default() },
            ],
            ..Default::default()
        };
        assert_eq!(conversation_availability(&q), ConversationAvailability::Exhausted);
