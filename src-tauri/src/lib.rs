        ...
        if let Some(tray) = app2.tray_by_id(TRAY_ID) {
            ...
            let tip = match store::get_state(&paths) {
                Ok(s) => {
                    ...
                    format!("Z-Accounts · {cur}")
                }
                Err(_) => "Z-Accounts".into(),
            };
...
#[tauri::command]
async fn get_recent_model_status() -> Option<model_status::ModelActivity> {
    model_status::recent(&Paths::detect())
}
...
async fn open_settings(app: AppHandle) -> Result<(), String> {
    ...
    .theme(Some(if load_settings(&Paths::detect()).theme() == "light" { tauri::Theme::Light } else { tauri::Theme::Dark }))
    .background_color(if load_settings(&Paths::detect()).theme() == "light" { tauri::window::Color(248, 249, 251, 255) } else { tauri::window::Color(31, 32, 34, 255) })
    .inner_size(w, h)
...
fn start_auto_switch_monitor(app: AppHandle) {
    std::thread::spawn(move || {
        ...
        loop {
            ...
            if pending_guard().is_some() || last_check.is_some_and(|at| at.elapsed() < CHECK_INTERVAL) {
                continue;
            }
            last_check = Some(Instant::now());

            let Ok(snapshot) = store::get_state(&paths) else { exhausted_once = None; continue };
            let Some(active_id) = snapshot.active_account_id.clone() else { exhausted_once = None; continue };
            if !snapshot.zcode_running || snapshot.accounts.len() < 2 {
                exhausted_once = None;
                continue;
            }
            let Ok(active_quota) = store::live_quota(&paths) else { exhausted_once = None; continue };
            if quota::conversation_availability(&active_quota) != quota::ConversationAvailability::Exhausted {
                exhausted_once = None;
                notified = None;
                continue;
            }
            if exhausted_once.as_deref() != Some(active_id.as_str()) {
                exhausted_once = Some(active_id);
                continue;
            }

            let Some(active_index) = snapshot.accounts.iter().position(|a| a.id == active_id) else { exhausted_once = None; continue };
            let mut target_id = None;
            for offset in 1..snapshot.accounts.len() {
                let candidate = &snapshot.accounts[(active_index + offset) % snapshot.accounts.len()];
                if !candidate.has_config { continue; }
                let Ok(candidate_quota) = store::account_quota(&paths, &candidate.id) else { continue };
                if quota::conversation_availability(&candidate_quota) == quota::ConversationAvailability::Available {
            ...
            // Recheck after network calls so a manual switch or disabling the
            // feature cannot be overwritten by a stale monitoring round.
            let result = {
                let _guard = store_guard();
                match store::get_state(&paths) {
                    Ok(now) if load_settings(&paths).auto_switch()
                        && now.zcode_running
            ...
            match result {
                Some(Ok(switched)) => {
                    ...
                    if switched.switched {
                        let _ = app.emit("auto-switch-result", json!({
                            "status": "switched",
                            "name": switched.name,
                ...
                Some(Err(error)) => {
                    ...
                    if notified.as_deref() != Some(active_id.as_str()) {
                        let _ = app.emit("auto-switch-result", json!({ "status": "error", "error": error }));
                        notified = Some(active_id);
                    }
                    exhausted_once = None;
                }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let theme = if store::load_settings(&Paths::detect()).theme() == "light" { tauri::Theme::Light } else { tauri::Theme::Dark };
            for window in app.webview_windows().values() {
