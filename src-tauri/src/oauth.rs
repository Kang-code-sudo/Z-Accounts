...
fn local_flow_deadline(expires_at_sec: f64, server_date_ms: Option<i64>, now_ms: u128) -> Option<u128> {
    if !expires_at_sec.is_finite() || expires_at_sec <= 0.0 {
        return None;
    }
    let expires_at_ms = (expires_at_sec * 1000.0) as i128;
    let reference_ms = server_date_ms.map(i128::from).unwrap_or(now_ms as i128);
    let server_remaining = expires_at_ms - reference_ms;
    // The product service can have a clock offset from Windows. Its HTTP Date
    // and expires_at share a clock, so use their difference as a relative TTL.
    // A missing/inconsistent Date falls back to the documented five-minute flow.
    let remaining_ms = if server_remaining > 0 && server_remaining <= (FLOW_TIMEOUT_MS as i128 * 2) {
        (server_remaining as u64).min(FLOW_TIMEOUT_MS)
    } else {
        FLOW_TIMEOUT_MS
    };
    Some(now_ms.saturating_add(u128::from(remaining_ms)))
}
...
fn init_flow_at(url: &str, provider: &str, mid: &str) -> Result<FlowInit, String> {
    ...
    let response = match req.send_json(json!({ "provider": provider })) {
        Ok(response) => response,
        Err(ureq::Error::Status(status, response)) => {
            // ureq treats non-2xx responses as errors. Read the server's short
            // diagnostic instead of discarding it as an opaque HTTP failure.
            let body = response.into_string().unwrap_or_default();
            let message = serde_json::from_str::<Value>(&body).ok()
                .and_then(|v| v.get("msg").and_then(Value::as_str).map(str::to_owned))
                .filter(|s| !s.trim().is_empty())
                .unwrap_or_else(|| crate::i18n::tr("err.oauth.init_server_unknown"));
            return Err(crate::i18n::trf("err.oauth.init_http", &[
                ("status", &status.to_string()), ("msg", &message),
            ]));
        }
        Err(e) => return Err(crate::i18n::trf("err.oauth.init", &[("e", &e.to_string())])),
    };
    let server_date_ms = response.header("Date")
    ...
    let expires_at_sec = data.get("expires_at").and_then(|x| x.as_f64()).ok_or_else(invalid)?;
    ...
    // embedded window never reaches our navigation handler.
    let redirect_key = match provider {
        "zai" => Some("redirect_uri"),
        "bigmodel" => Some("redirect"),
        _ => None,
    };
    if let Some(key) = redirect_key {
        let existing: Vec<(String, String)> = auth_url
            .query_pairs()
            .filter(|(k, _)| k != key)
        ...
        query.append_pair(key, &bridge_redirect_uri());
    }
    let state = auth_url
        .query_pairs()
        .find(|(k, _)| k == "state")
    ...
    let now = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0);
    let expires_at_ms = local_flow_deadline(expires_at_sec, server_date_ms, now).ok_or_else(invalid)?;
...
mod tests {
    ...
    #[test]
    fn oauth_flow_uses_server_relative_expiry_when_clocks_differ() {
        let local_now = 1_790_201_618_000u128;
        let server_now = 1_790_158_418_000i64;
        let server_expires = 1_790_158_718.0;
        assert_eq!(
            local_flow_deadline(server_expires, Some(server_now), local_now),
            Some(local_now + 300_000),
        );
    }

    #[test]
    fn oauth_flow_caps_expiry_and_rejects_invalid_timestamp() {
        let now = 1_790_201_618_000u128;
        assert_eq!(local_flow_deadline(1_790_158_718.0, None, now), Some(now + 300_000));
        assert_eq!(local_flow_deadline(f64::NAN, None, now), None);
    }

    #[test]
    #[ignore = "contacts the live ZCode OAuth service"]
    fn live_oauth_init_accepts_skewed_server_clock() {
        let flow = init_flow("zai", &uuid::Uuid::new_v4().to_string()).unwrap();
        assert!(flow.authorize_url.starts_with("https://"));
        assert!(!flow.state.is_empty());
        assert!(flow.expires_at_ms > std::time::SystemTime::now()
