...
pub struct Paths {
    /// OS user home: switcher snapshots and ZCode's credential cipher key use this.
    pub home: PathBuf,
    /// ZCode may relocate its data with ZCODE_DATA_BASE_DIR or HOME on Windows.
    pub zcode_data_base: PathBuf,
}
...
pub(crate) fn pick_zcode_data_base(
    zswitch: Option<PathBuf>,
    zcode_data_base: Option<PathBuf>,
...
impl Paths {
    pub fn detect() -> Paths {
        let zswitch = env_path("ZCODE_SWITCH_HOME");
        let userprofile = env_path("USERPROFILE");
        let home_env = env_path("HOME");
        let home = pick_home(
            zswitch.clone(),
            userprofile.clone(),
            home_env.clone(),
        );
        let zcode_data_base = pick_zcode_data_base(
            zswitch,
            env_path("ZCODE_DATA_BASE_DIR"),
            home_env,
            userprofile,
        );
        Paths { home, zcode_data_base }
    }

    pub fn zcode_root(&self) -> PathBuf { self.zcode_data_base.join(".zcode") }
    ...
    pub fn live_file(&self) -> PathBuf { self.zcode_root().join("v2").join("credentials.json") }
    pub fn live_config(&self) -> PathBuf { self.zcode_root().join("v2").join("config.json") }
    pub fn live_telemetry(&self) -> PathBuf { self.zcode_root().join("v2").join("telemetry-state.json") }
    pub fn live_setting(&self) -> PathBuf { self.zcode_root().join("v2").join("setting.json") }
    pub fn live_plan_cache(&self) -> PathBuf { self.zcode_root().join("v2").join("coding-plan-cache.json") }
...
impl Settings {
    ...
    pub fn theme(&self) -> &str { if self.theme.as_deref() == Some("light") { "light" } else { "dark" } }
...
#[cfg(windows)]
fn installed_zcode_path() -> Option<String> {
    ...
    PATH.get_or_init(|| {
        for hive in [
            ...
            let Ok(output) = no_window("reg").args(["query", hive, "/s"]).output() else {
                continue;
            };
            let text = String::from_utf8_lossy(&output.stdout);
            if let Some(path) = zcode_path_from_uninstall_registry(&text) {
                return Some(path);
            }
        }
        None
...
#[cfg(windows)]
fn zcode_path_from_uninstall_registry(text: &str) -> Option<String> {
    let mut display_name = String::new();
    let mut install_location = String::new();
    let mut display_icon = String::new();
    let mut uninstall_string = String::new();
    let mut candidates = Vec::new();
    let mut finish_entry = |display_name: &str, install_location: &str, display_icon: &str, uninstall_string: &str| {
        let name = display_name.trim().to_ascii_lowercase();
        if !name.starts_with("zcode") || name.contains("switch") {
            return;
        }
        if let Some(dir) = normalize_zcode_path(install_location) {
            candidates.push(PathBuf::from(dir).join("ZCode.exe"));
        }
        for source in [display_icon, uninstall_string] {
            let source = source.trim().trim_start_matches('"');
            let end = source.to_ascii_lowercase().find(".exe").map(|i| i + 4)
                .or_else(|| source.to_ascii_lowercase().find(".ico").map(|i| i + 4));
            if let Some(end) = end {
                if let Some(dir) = Path::new(&source[..end]).parent() {
                    candidates.push(dir.join("ZCode.exe"));
                }
            }
        }
    };
    for line in text.lines() {
        ...
        if line.starts_with("HKEY_") {
            finish_entry(&display_name, &install_location, &display_icon, &uninstall_string);
            display_name.clear();
            install_location.clear();
            display_icon.clear();
            uninstall_string.clear();
            continue;
        }
        let Some((key, value)) = line.split_once("REG_SZ").or_else(|| line.split_once("REG_EXPAND_SZ")) else {
            continue;
        };
        match key.trim() {
            "DisplayName" => display_name = value.trim().to_string(),
            "InstallLocation" => install_location = value.trim().to_string(),
            "DisplayIcon" => display_icon = value.trim().to_string(),
            "UninstallString" => uninstall_string = value.trim().to_string(),
            _ => {}
        }
    }
    finish_entry(&display_name, &install_location, &display_icon, &uninstall_string);
...
pub fn effective_zcode_path(paths: &Paths) -> (String, bool) {
    let mut candidates = Vec::new();
    #[cfg(windows)]
    if let Some(path) = installed_zcode_path() {
        candidates.push(path);
    }
    candidates.extend(client_path_candidates(std::env::consts::OS));
    effective_zcode_path_in(paths, &candidates)
}

fn effective_zcode_path_in(paths: &Paths, candidates: &[String]) -> (String, bool) {
    let s = load_settings(paths);
    if let Some(p) = s.zcode_path.as_ref().filter(|p| PathBuf::from(p).is_file()) {
        return (p.clone(), true);
    }
    for c in candidates {
...
pub fn switch_to(paths: &Paths, id: &str, force: bool, restart: bool, hot: bool) -> Result<SwitchResult, String> {
    ...
    if already {
        ...
        // The credentials on disk may already match while a long-running Electron
        // process still holds the previous account in memory.  The confirmation in
        // the UI passes force=true specifically to allow that process to be closed.
        // Ignoring force here made a same-account retry look successful without ever
        // making ZCode reload credentials.
        ...
        let (launched, launch_error) = if restart && !zcode_running() {
            launch_after_switch(paths)
        } else {
            (false, None)
        };
    ...
    let (launched, launch_error) = if restart {
        launch_after_switch(paths)
    } else {
        (false, None)
    };
...
fn launch_after_switch(paths: &Paths) -> (bool, Option<String>) {
    let (path, ok) = effective_zcode_path(paths);
    if !ok {
        return (false, Some(trf("err.zcode.path_invalid_hint", &[("p", &path)])));
    }
    match launch_zcode(&path) {
        Ok(()) => (true, None),
        Err(e) => (false, Some(e)),
    }
}
...
pub fn get_state(paths: &Paths) -> Result<AppState, String> {
    ...
    // ZCode can add encrypted, account-specific provider keys after startup.
    // The snapshot's whole-file hash then changes even though the login identity
    // is still the selected account.
    let active_account_id = live
        .as_ref()
        .filter(|_| live_logged_in)
        .and_then(|v| find_same_login(v, live_hash.as_deref().unwrap_or_default(), &accounts, &paths.home))
    ...
    let summaries = accounts
        ...
        .map(|a| AccountSummary {
            ...
            is_active: active_account_id.as_deref() == Some(a.id.as_str()),
...
mod path_tests {
    ...
    #[test]
    fn zcode_data_base_follows_zcode_environment_precedence() {
        let user = Some(PathBuf::from("user-profile"));
        let home = Some(PathBuf::from("home-env"));
        let data = Some(PathBuf::from("data-override"));
        assert_eq!(pick_zcode_data_base(None, data.clone(), home.clone(), user.clone()), PathBuf::from("data-override"));
        assert_eq!(pick_zcode_data_base(None, None, home, user.clone()), PathBuf::from("home-env"));
        assert_eq!(pick_zcode_data_base(None, None, None, user), PathBuf::from("user-profile"));
        assert_eq!(pick_zcode_data_base(Some(PathBuf::from("sandbox")), data, None, None), PathBuf::from("sandbox"));
    }

    #[test]
    fn switcher_snapshots_and_zcode_credentials_can_have_different_roots() {
        let paths = Paths {
            home: PathBuf::from("user-profile"),
            zcode_data_base: PathBuf::from("home-env"),
        };
        assert_eq!(paths.store_dir(), PathBuf::from("user-profile").join(".zcode-switch"));
        assert_eq!(paths.live_file(), PathBuf::from("home-env").join(".zcode").join("v2").join("credentials.json"));
    }
}
